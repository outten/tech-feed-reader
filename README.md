...
> **Status: Tier 2 complete.** Add feeds at `/feeds`, refresh manually or via `make scheduler`. Teaser feeds (HN, Lobsters) auto-fall back to a readability fetch. `/article/:uid` shows an auto-extractive summary; "Summarize with Claude" upgrades to an LLM summary via the official `anthropic` Ruby SDK (`claude-opus-4-7`). `/dashboard` now charts articles-per-day for the last 30 days alongside top-active-feeds and top-tags-this-week panels. Mark read / bookmark / archive, filter by state or tag, full-text search via SQLite FTS5. CLI: `make refresh-feed FEED=<id-or-url>`, `make refresh-feeds`, `make scheduler`. Tier 3 (export, OPML import, topic clustering, recommendations) is next.
