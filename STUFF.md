...
## [x] 112. Trivia game not working from the sidekiq cron job

The trivia game is not working in production or dev when ran from sidekiq cron job. It does work when run from the command line via "make generate-trivia".

**Shipped.** The original suspect (a stale/duplicate Sidekiq worker process raising `NameError`) didn't hold up once checked against the Droplet's actual `sidekiq` logs — `GenerateTriviaWorker` ran to completion every day, no exception, no dead job. The real bug, confirmed against the production database: `TriviaStore.fetch_source_articles` (`app/games/trivia_store.rb`) trusted `published_at DESC LIMIT 20` with no content-quality check. Feed 254 ("Three Word Phrase", a webcomic) reports `published_at` for ~80 items clustered at/near "now" — a couple even dated in the future (`2026-08-01`, `2026-09-16`) — while `content_text` is a handful of words of comic dialogue (`"*cough*"`, 4–60 chars). These content-free, fake-recent rows dominated the top-20 window on 07-22 and 07-23, starving Claude's context down to ~1,700 characters — too little for 3+ valid trivia questions. `ensure_today!` returned `nil` and the worker logged a warn and exited normally, so no exception meant no retry, no dead job, and the existing dead-job ntfy alert never fired — a silent failure that looked like "cron is broken" because a manual CLI run at a later moment (once real news had published past the pinned timestamp) always recovered. Fix: added a `MIN_ARTICLE_CHARS` (100) floor to `fetch_source_articles`'s WHERE clause, and `GenerateTriviaWorker` now pushes a `Notifier` alert whenever `ensure_today!` returns `nil` so a failed/skipped generation is visible same-day. Verified against the real historical production data (the 07-22 01:30 UTC window goes from 345 chars of junk to 60,207 chars of real news with the fix applied) plus 3 new specs; full suite 1742/0. `NTFY_URL` was found unset on the Droplet — flagged for the user to configure, not automated. Shipped in v1.1.25, PR #256.


## [x] 113. Expand YouTube Listings.
...
Full plan in `openspec/changes/archive/2026-09-18-ios-app/` (proposal, design, specs, tasks). A token-authenticated `/api/v1/*` JSON API (feeds, articles, subscriptions, read-state, stocks, sports, podcasts/YouTube, digests/triage, account management, and more) alongside the existing cookie-session routes, and a SwiftUI app under `ios/` (built via XcodeGen, `ios/project.yml`) that builds and runs on both iPhone and iPad Simulators. Sign-up/login are covered by recovery-code login (native) + a browser-handoff to the existing web `/sign-up` flow, since Associated Domains (needed for native passkey UI) aren't set up yet — deferred to Phase 2 pending a production domain + Apple Developer Team ID. See `openspec/changes/archive/2026-09-18-ios-app/design.md` for the full rationale.

**Shipped.** Phases 1, 3-17 merged via PR #261 (squash-merged into `main` at `5f6729e`): the full mobile API surface plus a SwiftUI app covering reading, discovery, stocks, sports, podcasts/YouTube, AI features (digests/triage), and account management, with visual parity passes against the web app's card/grid styling. Phase 2 (native passkey login) remains deferred, blocked on a production domain + Apple Developer Team ID — the app currently authenticates via recovery-code login instead.
