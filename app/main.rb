...
class TechFeedReader < Sinatra::Base
  ...
  helpers do
    ...
    def format_market_cap(val, currency = nil)
      ...
      prefix = cur == 'USD' ? '$' : "#{cur} "
      v = val.to_i
      if    v >= 1_000_000_000_000 then format("#{prefix}%.2fT", v / 1_000_000_000_000.0)
      elsif v >= 1_000_000_000     then format("#{prefix}%.2fB", v / 1_000_000_000.0)
      elsif v >= 1_000_000         then format("#{prefix}%.2fM", v / 1_000_000.0)
      else  "#{prefix}#{v}"
      end
    end
