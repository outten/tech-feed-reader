...
- **Phase 13 — Player parity (roadmapped, not yet specced)**: the current mini-player is play/pause and a read-only progress bar. Missing: seek/scrub, ±15s/±30s skip, playback speed, elapsed/total time display, resume-from-last-position (the web persists this in `localStorage`; iOS has no equivalent yet), and — the one worth flagging as a real gap rather than a nicety — `UIBackgroundModes: [audio]` is already declared and background playback already works, but there's no `MPNowPlayingInfoCenter`/`MPRemoteCommandCenter` wiring, so backgrounded audio has no lock-screen/CarPlay/AirPods controls at all today.
