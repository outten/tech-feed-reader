...
- **Phase 6 — Sports**. The largest remaining phase — its own dedicated schema (`sports_leagues`/`sports_teams`/`sports_matches`/`sports_players`/`sports_standings`/`sports_follows`), a 128-league/339-team static catalog (`app/sports_catalog.rb`), and web routes spanning browse/manage/follow/standings/calendar. Split, same reasoning as every prior phase split:
  ...
  - **Phase 6b (deferred)**: tennis ATP/WTA rankings (`/sports/tennis` — its own self-contained ranking system) and Wikipedia league summaries (nice-to-have text blob on league pages). Neither blocks core browse/follow/view. (The third original 6b item — surfacing the per-user `/username/sports/calendar.ics` URL as an "Add to Calendar" action — shipped in Phase 10 once `GET /api/v1/account` existed to supply the username.)
...
### Phases 11-15 — closing the gaps a systematic audit found (added after Phase 10 shipped)

Phases 1-10 covered every *area* of the web app, but a full route-by-route, view-by-view audit (prompted by the user flagging that article feedback, YouTube depth, and source links were missing) found that several already-"done" phases are materially thinner than their web counterparts, and that a few whole web surfaces — the home dashboard, the plain "all articles" reading river, real player controls — were never phased at all. Corrections the audit surfaced, worth recording so they don't get re-litigated:

- **Summarize buttons don't exist on the web UI anymore** (STUFF #59 removed them from `views/article.erb`) — only the *cached* summary display remains for a signed-in user; the `POST /article/:uid/summarize*` routes survive for scripted/curl use only. Phase 11 specs *displaying* a cached summary, not a summarize action.
- **There is no comment feature anywhere in the app.** STUFF.md #1's "Comment and Reload button" was a *layout* complaint about button spacing on a long-gone version of the article page, not a feature — don't spec comments.
- **Social sharing doesn't exist on the web either.** Not a parity gap; if it happens on iOS it's a native-platform opportunity (`ShareLink`), tracked separately from this parity effort if wanted.

- **Phase 11 — Article detail parity (this pass, fully specced/tasked below)**: the article detail screen is currently a content renderer, not an article page. Adds the metadata header (hero image, feed name, author, relative time, reading time / episode duration), a source link (open the original URL), 👍/👎 feedback, mute-author/mute-keyword shortcuts (reusing the existing mute-rules endpoint, just surfaced inline), tag chips (view/apply/remove — the first *write* path for tags on mobile), the cached summary block, and a real mark-unread toggle (today's `markRead()` only ever sets `read: true`). Backend need is still small: one new feedback endpoint, one new tag-apply/remove endpoint pair, and adding the already-cached `summary` to the existing article-detail response — everything else (image URL, author, feedback value, duration) is already in the payload the iOS `Article` model just doesn't decode yet.
- **Phase 12 — The reading river (this pass, fully specced/tasked below)**: there is currently no "All Articles" screen at all — the sidebar has no entry for it, so launching the app with nothing else selected is a dead end. Adds an all-articles screen with state/kind/topic filters and pagination (the API already supports `page`, iOS just never passes it), plus `sort=relevance` — the personalized For-You ranker (`Recommendation::ForYou`), which iOS currently cannot request *or* feed signal into (Phase 11's feedback endpoint was the prerequisite: the ranker partly weights on 👍/👎, and Phase 11 already shipped it). Backend need: `GET /api/v1/articles` gains a `kind` filter (podcast/all — the store method already supports it, same as `topic` in Phase 8a) and a `sort=relevance` branch that calls `Recommendation::ForYou.score_window` exactly like the web `/articles` route, forcing `state=unread` while active (matching the web behavior — the ranker only scores unread articles).
- **Phase 13 — Player parity (this pass, fully specced/tasked below)**: the current mini-player is play/pause and a read-only progress bar. Adds seek/scrub, ±15s/±30s skip (matching the web app's `SKIP_BACK_S`/`SKIP_FWD_S`), playback speed (1×/1.25×/1.5×/1.75×/2×, same options as the web `<select>`), elapsed/total time display, resume-from-last-position (stored in `UserDefaults`, mirroring the web app's per-episode `localStorage` key — both are local-only, never synced through the account, so this is a direct architectural match rather than a compromise), and — the one worth flagging as a real gap rather than a nicety — `UIBackgroundModes: [audio]` is already declared and background playback already works, but there was no `MPNowPlayingInfoCenter`/`MPRemoteCommandCenter` wiring, so backgrounded audio had no lock-screen/CarPlay/AirPods controls at all before this phase. No backend changes — everything here is local playback state, matching the web app's own local-only (non-account-synced) approach.
- **Phase 14 — Home / "What's On Today" (this pass, fully specced/tasked below)**: the web's `/` dashboard (today's unread/bookmarked stats, live matches, sports today, to-read/listen/watch-today sections, continue-listening/watching) had no iOS equivalent — the app opened straight to "Select an Item." Adds a `GET /api/v1/home` endpoint bundling exactly what the web's `load_whats_on_today!` helper assembles (stats, today's sports matches + live matches with team info merged in, today's reading/listening/watching lists), and a Home screen that becomes the sidebar's default landing item. **Scoped down from the web in two ways, both worth stating rather than silently dropping**: (1) no YouTube-channel fallback padding when today's actual video count is thin (the web's "top up to 10" nicety) — the mobile list just shows what published today, possibly short of 10; (2) "Continue Watching" (YouTube resume position) isn't included — Phase 13 only added resume-position tracking for the audio player, not the YouTube `WKWebView` embed, so only "Continue Listening" (podcasts) is built. Tracking YouTube watch position would need a JS bridge into the embed and is deferred, not silently dropped.
- **Phase 15 — Discovery odds and ends (this pass, fully specced/tasked below)**: smaller web affordances with no iOS equivalent yet. Adds: bus mode (`GET /api/v1/articles/bus?max_minutes=`, mirroring the web `/bus` route's constants), "I Feel Lucky" (`GET /api/v1/articles/lucky`, wraps `ArticlesStore.random`), per-feed refresh-now (`POST /api/v1/feeds/:id/refresh`) + feed-weight controls (`POST /api/v1/feeds/:id/weight`, and `GET /api/v1/feeds` now also returns each feed's current weight — a **modification** to the existing `mobile-api` feeds-list response, additive/backward-compatible), tag-rule *creation and deletion* (`POST`/`DELETE /api/v1/tags` — Phase 11 only added apply/remove of *existing* tags to one article), first-run onboarding (`GET /api/v1/onboarding/chips` + `POST /api/v1/onboarding/subscribe`, mirroring the web `/welcome` flow — a brand-new iOS user currently gets an empty sidebar with no guidance), and account data export (`GET /api/v1/account/export`, wraps the same `AccountExport.for_user` the web `/account/export.json` route uses; the iOS side offers it via the native share sheet, `ShareLink`, rather than a download prompt).

Each of Phases 12-15 gets its own `specs/<capability>/spec.md` and `tasks.md` section when its turn comes, same as every phase before it.
