...
- **Phase 15 — Discovery odds and ends (roadmapped, not yet specced)**: smaller web affordances with no iOS equivalent yet — bus mode (short-podcast filter), "I Feel Lucky" (random cross-type sample), per-feed refresh-now + feed-weight controls, tag-rule *creation* (Phase 11 adds apply/remove of existing tags to an article, not authoring new rules), first-run `/welcome` onboarding (a brand-new iOS user currently gets an empty sidebar with no guidance), and the account data-export (`/account/export.json`, a privacy-policy commitment on web).
