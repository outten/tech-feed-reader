...
- **Phase 12 — The reading river (roadmapped, not yet specced)**: there is currently no "All Articles" screen at all — the sidebar has no entry for it, so launching the app with nothing else selected is a dead end. Needs an all-articles screen with state/kind/topic filters and pagination (the API already supports `page`, iOS just never passes it), plus `sort=relevance` — the personalized For-You ranker (`Recommendation::ForYou`), which iOS currently cannot request *or* feed signal into (Phase 11's feedback endpoint is a prerequisite: the ranker partly weights on 👍/👎).
