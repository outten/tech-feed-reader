...
- `mobile-account`: Phase 10 — display name edit, recovery-code regeneration, passkey list + revoke (add still waits on Phase 2), delete account, and (a cheap bonus once the current-user endpoint exists) surfacing the public sports-calendar `.ics` subscription URL from Phase 6b's deferred list. This closes out the originally-roadmapped phases; remaining work is the deferred 4b/6b/8b items plus the still-open Chat/Admin questions, all in `design.md`.
