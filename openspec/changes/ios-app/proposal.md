...
- `mobile-stocks`: Phase 7 — symbol search, quote detail + per-symbol news, follow/unfollow (which also subscribes to the symbol's news feed, matching the web app), and a ticker (followed symbols + major world indices). Later phases (misc content, AI features, account management) are roadmapped in `design.md` but not yet specced — each gets its own capability spec when its phase starts.
