...
- `mobile-podcasts-youtube`: Phase 5 — podcast episode browsing + native audio playback (a persistent mini-player, not an embedded web `<audio>` tag — matches the web app's "survives navigation" mini-player while using the platform's native AVPlayer), and YouTube channel browsing + embedded video playback. Later phases (Sports, Stocks, misc content, AI features, account management) are roadmapped in `design.md` but not yet specced — each gets its own capability spec when its phase starts.
