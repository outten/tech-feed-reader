...
RSpec.describe Digests do
  ...
  describe '.compose' do
    it 'returns a zero-count Result when no unread articles fall in the window' do
      result = Digests.compose(1, now: now)
    ...
    it 'includes only UNREAD articles within the window' do
      ...
      ReadStateStore.mark_read(1, ArticlesStore.find_by_uid('bbbbbbbbbbbb')['id'], read: true)

      result = Digests.compose(1, window_hours: 24, now: now)
    ...
    it 'honours the limit argument' do
      ...
      result = Digests.compose(1, limit: 3, now: now)
    ...
    it 'subject reflects count + day; text includes "last Nh"' do
      ...
      result = Digests.compose(1, window_hours: 12, now: now)
  ...
  describe '.generate_and_store!' do
    it 'composes a Result and persists it via DigestStore, returning [id, result]' do
      ...
      id, result = Digests.generate_and_store!(1, now: now)
      expect(id).to be > 0
      expect(result.count).to eq(1)
  ...
  describe '.query_unread' do
    it 'returns rows joined with feed title + cached summaries' do
      ...
      rows = Digests.query_unread(1, window_hours: 24, limit: 5, now: now)
