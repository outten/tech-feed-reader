...
RSpec.describe TagsStore do
  describe '.add' do
    ...
    it 'rejects duplicate names via UNIQUE' do
      ...
      expect {
        TagsStore.add(name: 'ruby', match_kind: 'keyword', match_value: 'rails')
      }.to raise_error(SQLite3::ConstraintException, /UNIQUE/)
    end

    it 'rejects unknown match_kind via the schema CHECK' do
      expect {
        TagsStore.add(name: 'x', match_kind: 'bogus', match_value: 'x')
  ...
  describe '.find / .find_by_name / .all' do
    before do
      TagsStore.add(name: 'rails', match_kind: 'keyword', match_value: 'rails')
      TagsStore.add(name: 'ai',    match_kind: 'keyword', match_value: 'ai')
    end

    it '.all returns rows sorted by name' do
      expect(TagsStore.all.map { |t| t['name'] }).to eq(%w[ai rails])
    end

    it '.find_by_name lookup' do
      expect(TagsStore.find_by_name('rails')).not_to be_nil
      expect(TagsStore.find_by_name('nope')).to be_nil
    end

    it '.find by id' do
      id = TagsStore.find_by_name('ai')['id']
      expect(TagsStore.find(id)['name']).to eq('ai')
    end
  end
  ...
  describe '.remove' do
    it 'cascades to article_tags' do
      ...
      expect(TagsStore.tags_for_article(article_id).map { |t| t['name'] }).to include('ruby')

      TagsStore.remove(tag['id'])
      expect(TagsStore.tags_for_article(article_id)).to be_empty
    end
  end
  ...
  describe '.tags_for_articles bulk lookup' do
    it 'returns a hash keyed by article_id' do
      feed = FeedsStore.add(url: 'https://example.com/feed.rss')
      tag1 = TagsStore.add(name: 'tag1', match_kind: 'feed_id', match_value: feed['id'].to_s)
      tag2 = TagsStore.add(name: 'tag2', match_kind: 'feed_id', match_value: feed['id'].to_s)
      ArticlesStore.import(feed_id: feed['id'], entries: [{
